...
int main(int argc, char** argv)
{
    ...
    if (parser.isSet(QStringLiteral("my-id"))) {
        ...
        if (parser.isSet(QStringLiteral("share"))) {
            ...
            blockOnReply(QDBusConnection::sessionBus().asyncCall(msg));
        } else if(parser.isSet(QStringLiteral("hello"))) {
            QDBusMessage msg = QDBusMessage::createMethodCall(QStringLiteral("org.kde.kdeconnect"), "/modules/kdeconnect/devices/"+device+"/hello", 	QStringLiteral("org.kde.kdeconnect.device.hello"), QStringLiteral("sendHello"));
